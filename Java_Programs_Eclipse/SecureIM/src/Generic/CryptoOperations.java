...
public class CryptoOperations {
	...
	private static final String RSA_CIPHER = "RSA/ECB/NOPadding";
	...
	public static String generateValidationHash(String password) {
		try {
			return String.format("%x",new BigInteger(hash(password+hash(password)).getBytes(CryptoOperations.CHARSET)));
	...
	public static String hash(String value) {
		try {
			...
			return new String(sha1.digest(value.getBytes(CryptoOperations.CHARSET)),
		...
		throws HmacException 
	{
		...
		return MessagePackUnpack.pack(params);
	}
	...
	throws HmacException 
	{
		final ArrayList<String> params = MessagePackUnpack.unpack(message);
	...
	public static String hmacGenerate(SecretKey key, String msg)
		...
		try {
			...
			mac.init(key);
			return new String(mac.doFinal(msg.getBytes(CryptoOperations.CHARSET)),
					CryptoOperations.CHARSET);
	...
	public static String rsaEncrypt(PublicKey key, String msg)
		...
		try {
			...
			return new String(cipher.doFinal(msg.getBytes(CryptoOperations.CHARSET)), CryptoOperations.CHARSET);
		} catch (Exception e) {
	...
	public static SecretKey rsaDecryptAESKey(PrivateKey key, String msg) throws 
																			InvalidKeyException, 
		...
		return (SecretKey) RSACipher.unwrap(msg.replaceAll("\0", "").getBytes(CryptoOperations.CHARSET), "AES", Cipher.SECRET_KEY);
	}
	...
	public static SecretKey aesGenerateKey() throws KeyCreationException {
		try {
			...
			kgen.init(128);
			return kgen.generateKey();//aesCreateKey(kgen.generateKey().getEncoded());
		} catch (Exception e) {
	...
	public static String aesEncrypt(SecretKey key, String msg)
		...
		try {
			...
			return new String(initVectorBytes, CryptoOperations.CHARSET)
							+ new String(cipher.doFinal(msg.getBytes(CryptoOperations.CHARSET)), CryptoOperations.CHARSET);
		} catch (Exception e) {
			throw new EncryptionException(e);
		}
	}
	...
	public static String aesDecrypt(SecretKey key, String msg)
		...
		try {
			final IvParameterSpec initVector = new IvParameterSpec(msg
					.substring(0, IV_LENGTH).getBytes(CryptoOperations.CHARSET));
			...
			return new String(
					cipher.doFinal(msg.getBytes(CryptoOperations.CHARSET)),
					CryptoOperations.CHARSET);
		} catch (Exception e) {
			throw new DecryptionException(e);
		}
